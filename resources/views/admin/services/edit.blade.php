<x-layouts.app title="Edit Service">
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-outfit tracking-tight">Edit Service</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Updating: <span class="font-bold text-brand-500">{{ $service->title }}</span></p>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="3" required
                                    class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white">{{ old('description', $service->description) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="long_description" id="long_description" rows="8"
                                    class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white">{{ old('long_description', $service->long_description) }}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="glass-card p-6 sm:p-8 space-y-6">
                        ...
                        <div>
                            <label for="icon" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">SVG Icon Code</label>
                            <textarea name="icon" id="icon" rows="4"
                                class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-xs font-mono focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white">{{ old('icon', $service->icon) }}</textarea>
                        </div>

                        <div>
                            <label for="image" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Featured Image</label>
                            @if($service->image)
                                <img src="{{ $service->image }}" class="w-full h-32 object-cover rounded-xl mb-4">
                            @endif
