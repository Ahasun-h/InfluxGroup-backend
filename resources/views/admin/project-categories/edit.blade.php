<x-layouts.app title="Edit Project Category">
    <div class="max-w-2xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-outfit tracking-tight">Edit Project Category</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Updating: <span class="font-bold text-brand-500">{{ $projectCategory->name }}</span></p>
            </div>
        </div>

        <form action="{{ route('admin.project-categories.update', $projectCategory) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="3"
                            class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white placeholder-gray-400">{{ old('description', $projectCategory->description) }}</textarea>
                        ...
                        <div class="space-y-3">
                            ...
                            <div id="svg-section" class="space-y-3">
                                <div class="flex gap-2">
                                    <textarea name="icon_svg" id="icon-svg" rows="3"
                                        ...
                                        placeholder="Paste your SVG code here...">{{ old('icon_svg', $projectCategory->icon) }}</textarea>
                                    <div id="svg-preview" class="w-12 h-12 bg-gray-100 dark:bg-surface-800 rounded-xl flex items-center justify-center border border-gray-200 dark:border-white/10 overflow-hidden">
                                        <span id="svg-display" class="text-2xl">{{ $projectCategory->icon ?: '⚡' }}</span>
                                    </div>
                                </div>
                            ...
                            <input type="hidden" name="icon" id="icon-input" value="{{ old('icon', $projectCategory->icon) }}">
                        </div>
                        ...
                        @if($projectCategory->image)
                            <div class="mt-3">
                                ...
                                <div class="relative inline-block">
                                    <img src="{{ $projectCategory->image }}" alt="{{ $projectCategory->name }}" class="h-32 w-auto rounded-lg border border-gray-200 dark:border-white/10 object-cover">
                                </div>
                                <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">Upload a new image to replace the current one</p>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="order" id="order" value="{{ old('order', $projectCategory->order) }}" min="0"
                                class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white placeholder-gray-400">
                            <p class="text-xs text-gray-500 mt-1">Lower numbers appear first</p>
                        </div>
                        <div class="flex items-center gap-3">
                            <input type="checkbox" name="is_active" id="is_active" {{ $projectCategory->is_active ? 'checked' : '' }}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const existingIcon = iconInput.value;
            if (existingIcon) {
                svgInput.value = existingIcon;
                svgDisplay.innerHTML = existingIcon;
            }
        });
    </script>
</x-layouts.app>
