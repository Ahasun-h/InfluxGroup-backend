<x-layouts.app title="Edit Category">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <a href="{{ route('admin.categories.index', ['area' => $category->service_area]) }}" class="inline-flex items-center text-sm font-medium text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
        ...
        <div class="glass-card p-8">
            <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white font-outfit mb-6">Edit Category</h1>
            ...
            <form method="POST" action="{{ route('admin.categories.update', $category) }}">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="service_area" name="service_area" required
                            ...
                            @foreach($serviceAreas as $value => $label)
                                <option value="{{ $value }}" {{ old('service_area', $category->service_area) == $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea id="icon" name="icon" rows="4"
                            ...
                            placeholder="<svg class='w-6 h-6' fill='none' stroke='currentColor' viewBox='0 0 24 24'>...</svg>">{{ old('icon', $category->icon) }}</textarea>
                ...
                <div class="mt-8 flex items-center justify-end gap-4">
                    <a href="{{ route('admin.categories.index', ['area' => $category->service_area]) }}" class="px-6 py-3 rounded-xl border border-gray-200 dark:border-surface-700 text-gray-700 dark:text-gray-300 font-medium hover:bg-gray-50 dark:hover:bg-surface-700 transition-all">
                        Cancel
                    </a>
