<x-layouts.app title="Edit Product Category">
    <div class="max-w-4xl mx-auto space-y-8">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white font-outfit tracking-tight">Edit Category</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400">Update category information.</p>
            </div>
        </div>

        <form action="{{ route('admin.product-categories.update', $productCategory) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <textarea name="description" id="description" rows="4"
                                    class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white placeholder-gray-400"
                                    placeholder="Brief description of this product category">{{ old('description', $productCategory->description) }}</textarea>
                                <x-input-error :messages="$errors->get('description')" class="mt-2" />
                            </div>

                            <div>
                                <label for="icon" class="block text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5">Icon (Emoji)</label>
                                <input type="text" name="icon" id="icon" value="{{ old('icon', $productCategory->icon) }}"
                                    class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white placeholder-gray-400"
                                    placeholder="e.g., ⚡ or 🔌">
                            ...
                            <div>
                                ...
                                <input type="number" name="order" id="order" value="{{ old('order', $productCategory->order) }}" min="0"
                                    class="w-full px-4 py-2.5 bg-gray-50/50 dark:bg-surface-900/50 border border-gray-200 dark:border-white/10 rounded-xl text-sm focus:ring-2 focus:ring-brand-500 outline-none transition-all dark:text-white placeholder-gray-400">
                            ...
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="is_active" id="is_active" {{ $productCategory->is_active ? 'checked' : '' }}
                ...
                <div class="space-y-6">
                    ...
                    <div class="glass-card p-6 space-y-4">
                        ...
                        <div class="bg-white dark:bg-surface-800 rounded-xl p-4 border-2 border-dashed border-gray-200 dark:border-white/10">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-white text-2xl">
                                    {{ $productCategory->icon ?: '⚡' }}
                                </div>
                                <div>
                                    <h4 class="font-bold text-gray-900 dark:text-white">{{ $productCategory->name }}</h4>
                                    <span class="text-xs text-gray-500">{{ $productCategory->products()->count() }} Products</span>
                                </div>
                            </div>
                        </div>
                    </div>
