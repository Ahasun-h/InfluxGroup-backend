    ...
    <script>
        ...
        window.editField = function(fieldName) {
            ...
            if (fieldName === 'image_url') {
                ...
                // Set URL values
                if (urlInput) urlInput.value = currentImageUrl;
                if (manualInput) manualInput.value = currentImageUrl;

                console.log('✓ Image values set');
        ...
        window.closeFieldModal = function() {
            ...
            if (typeof dropifyInstance !== 'undefined' && dropifyInstance && typeof dropifyInstance.destroy === 'function') {
                try {
        ...
        // Close modals on outside click and initialize
            ...
            if (fieldName === 'image_url') {
                ...
                if (dropifyInstance) {
                    try {
                        dropifyInstance.destroy();
                        console.log('✓ Previous Dropify instance destroyed');
                    } catch (e) {
                ...
                if (dropifyInput) {
                    // Clear previous value and reset
                    dropifyInput.value = '';
                    dropifyInput.setAttribute('data-default-file', currentImageUrl);
                    dropifyInput.style.display = '';
                }
                ...
                console.log('✓ Image values set:', {
                    'data-default-file': dropifyInput?.getAttribute('data-default-file'),
                    'url-input': urlInput?.value,
                    'manual-input': manualInput?.value
                });
                ...
                setTimeout(function() {
                    ...
                    if (checkDropifyReady()) {
                        try {
                            // Remove any existing Dropify wrapper
                            const existingWrapper = dropifyInput?.nextElementSibling;
                            if (existingWrapper && existingWrapper.classList.contains('dropify-wrapper')) {
                                existingWrapper.remove();
                            }

                            // Initialize Dropify with current image
                            dropifyInstance = window.$('#image-upload').dropify({
                                ...
                                tpl: {
                                    wrap:            '<div class="dropify-wrapper"><div class="dropify-message"><span class="file-icon" /></div><div class="dropify-preview" /><span class="dropify-errors" /><button class="dropify-clear">@{{ remove }}</button></div>',
                            ...
                            dropifyInstance.on('dropify.change', function(event, element) {
                                ...
                                if (file) {
                                    ...
                                    if (manualInput) manualInput.value = objectUrl;
                                    console.log('✓ Object URL created:', objectUrl);
                                }
                            });

                            dropifyInstance.on('dropify.ready', function() {
                                console.log('✓ Dropify ready with image');
                            });
                    ...
                    const manualUrlInput = document.getElementById('manual-image-url');

                    // Remove any existing event listeners by cloning
                    if (manualUrlInput) {
                        const newManualInput = manualUrlInput.cloneNode(true);
                        manualUrlInput.parentNode.replaceChild(newManualInput, manualUrlInput);
                        ...
                        newManualInput.addEventListener('input', function(e) {
                            const url = e.target.value;
                            debouncedUpdate(url);
                        });

                        // Handle paste event (immediate update)
                        newManualInput.addEventListener('paste', function(e) {
                            setTimeout(function() {
                                const url = e.target.value;
                                if (url && urlInput) {
                                    urlInput.value = url;
                        ...
                        newManualInput.addEventListener('blur', function(e) {
        ...
        console.log('editField function defined:', typeof window.editField);
